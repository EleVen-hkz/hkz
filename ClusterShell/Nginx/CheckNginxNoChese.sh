...
GET_IP
echo  "<STRESSRESULT>"
ALLPID=`ps -ef |grep -v grep |grep  nginx |awk '{print $2}'|sort|uniq`
...
if [[ ${NGINXPID} ]]; then
    for PID in ${NGINXPID}; do
        ...
        PORTS=`netstat -luntp |grep -w ${PID}|grep -w tcp|awk '{print $4}'|awk -F: '{print $NF}'`
    ...
    NGINXLOGS=`cat ${NGINXCONF} |sed -e 's/^[ \t]*//g'|grep -Ev "^#|^$"|grep -Ev "stderr|off" |grep -E "error_log|access_log" |awk '{print $2}'|awk -F\; '{print $1}'`
        ...
        sed -n '/upstream/,/}/p' "$NGINXCONF" | sed -n '$!p' | grep -v '#' | awk -F 'server ' '{print $2}' | grep -v '{' | awk -F ' ' '{print $1}'  > nginx1.txt
        sed -i 's/;/@@/g' nginx1.txt
        if [ -s nginx1.txt ];then
         for i in `awk '{print NR}' nginx1.txt`;do
               HANG=`sed -n "$i"p nginx1.txt`
                   if [ -n "$HANG" ];then
                         echo -n $HANG >> nginx2.txt
                   else
                echo -e "\n" >> nginx2.txt
             fi
         done
         sed -i '/^\s*$/d' nginx2.txt
         for i in `awk '{print NR}' nginx2.txt`;do
            HANG2=`sed -n "$i"p nginx2.txt`
            if [[ "$HANG2" =~ "@@" ]];then
               echo  `sed -n "$i"p nginx2.txt |  sed  "s/..$//g" | sed 's/^/[&/g' | sed 's/$/&]/g'` >>nginx3.txt
            else
              echo `sed -n "$i"p nginx2.txt | sed 's/^/[&/g' | sed 's/$/&]/g'` >>nginx3.txt
            fi
         done
         rm -rf nginx1.txt nginx2.txt
         BLA=`sed ':a;N;$!ba;s/\n//g' nginx3.txt`
         rm -rf nginx3.txt
        fi

        for i in ${PORTS}; do
            echo "${HOSTIP}:${i}##Nginx##nginx##${NGINXVER}##OneNode##${HOSTIP}$(printf ':%s' ${PORTS})${BLA}##${NGINXDIR}##$(printf '%s' ${NGINXLOGDIR})##${NGINXCONF}"
        done
    done
else
